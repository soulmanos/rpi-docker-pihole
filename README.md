...
4. Removed tini - Container lighttpd was failing for some reason - Trying without importing tini and using --init on docker run

> docker build --no-cache -t soulmanos/rpi-docker-pihole .

> docker run -p 53:53 -p 53:53/udp -p 80:80 -d --name pihole --init --dns=8.8.8.8 -e ADMIN_PASS="pihole" -e DNS1="8.8.8.8" -e DNS2="8.8.4.4" -e ServerIP="0.0.0.0" soulmanos/rpi-docker-pihole-no-tini:latest
